...
public class ClientUserServiceImpl implements ClientUserService {
    ...
    @Override
    @Transactional(readOnly = true)
    public ClientUserDTO findOne(Long id) {
        log.debug("Request to get ClientUser : {}", id);
        ClientUser clientUser = clientUserRepository.findOneWithEagerRelationships(id);
        return clientUserMapper.toDto(clientUser);
    }
    ...
    @Override
    public ClientUserDTO login(String phone, String password) {
        log.debug("Request to login ClientUser : {}", phone);
        ClientUser clientUser = clientUserRepository.findByPhone(phone);
        if (clientUser!=null&&password.equals(clientUser.getPassword())){
            return clientUserMapper.toDto(clientUser);
        }
        return null;
    }
    ...
    @Override
    public void uploadFigure(File file, Long id) {
